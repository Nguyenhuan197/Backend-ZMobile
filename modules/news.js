...
const connectSchema = require("../Schema/news");
...
const addNew = async (req, res, next) => {
    try {
        ...
        if (!name.trim() || !describe.trim()) return res.status(400).json({ mesage_vn: 'Thêm thất bại', mesage_en: 'More failures', status: false });
        const newProduct = new connectSchema({ name, img, describe });
